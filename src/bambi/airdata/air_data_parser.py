...
from typing import Any, Dict, Generator, List, Optional, TextIO, Union, Iterable, Sequence
...
class AirDataParser(AirDataParserInterface):
    ...
    @staticmethod
    def get_frames_video_offset(frames: Iterable[AirDataFrame], video_time: datetime.datetime,
                                max_timedelta: timedelta = timedelta(seconds=60)) -> int:
        ...
        for frame in frames:
            if frame.isVideo or frame.isPhoto:
                ...
                if diff < min_diff:
                    best_frame = frame
                    min_diff = diff
                else:  # time usually does not reverse
                    break

        if best_frame is None:
            raise ValueError(f"The AirData file [{file}] does not contain any frames")

        if min_diff > max_timedelta:
            raise ValueError(
                f"The found video offset is larger than the maximum timedelta ({min_diff} > {max_timedelta})")

        ms_offset = best_frame.time

        return ms_offset

    def get_file_video_offset(self, file: Union[str, TextIO, TextIOBase], video_time: datetime.datetime,
                              max_timedelta: timedelta = timedelta(seconds=60)) -> int:
        """
        ...
        :param max_timedelta: the maximum duration of the video (if no timezone given assume UTC time)
        :return: offset in milliseconds
        """
        return self.get_frames_video_offset(self.parse_yield(file), video_time, max_timedelta)
